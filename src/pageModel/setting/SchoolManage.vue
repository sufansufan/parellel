...
<script>
    export default {
        ...
        created() {
            const self= this;
            if(window.sessionStorage.getItem("areaNav")){ //已经加载过一次区域导航了
                this.location = JSON.parse(window.sessionStorage.getItem("areaNav"));
                this.getData();
            }else{ //没有加载过区域导航
                let objData={
                    staffId:JSON.parse(window.sessionStorage.getItem("staff")).staffId,
                };
                this.$axios.post(this.GlobalVal.httpLink.getAreaNav,JSON.stringify(objData)).then(
                    res=>{
                        if(res.data.code ==="10000"){
                            self.location = res.data.areaNav;
                            window.sessionStorage.setItem("areaNav",JSON.stringify(res.data.areaNav));
                        }else{
                            console.error(res);
                            this.$message({
                                type:"error",
