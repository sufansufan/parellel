...
<template>
  <div class="class-query table2">
    <el-table size="mini" :data="data" border style="width: 100%" ref="multipleTable" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="class_name" width="250" label="班级名称" fixed></el-table-column>
      ...
      <el-table-column prop="time_name" label="上课时间" width="100"></el-table-column>
      ...
      <el-table-column prop="order_status" label="状态" fixed></el-table-column>
      <el-table-column prop="remark" label="备注"></el-table-column>
      <el-table-column prop="staff_name" label="操作人"></el-table-column>
      <el-table-column label="操作" width="280" fixed="right">
        <template slot-scope="scope">
          <!-- <el-button size="mini" type="primary"
                               @click="handleOPencourse(scope.$index, scope.row)">开课
                    </el-button> -->
          <el-button size="mini" type="primary" @click="handleApply(scope.$index, scope.row)" :disabled="scope.row.orderstatus === '01011'">转班
          </el-button>
          <el-button size="mini" type="primary" @click="handleCourse(scope.$index, scope.row)">调课
          </el-button>
          <!-- <el-button size="mini" type="primary" @click="handlemoney(scope.$index, scope.row)" :disabled="scope.row.orderstatus === '转班转出'">退费
          </el-button> -->
          <el-button size="mini" type="primary" @click="printEdit(scope.$index, scope.row)">打印听课证
          </el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <transition name="el-zoom-in-center">
          <div v-if="classTimeLayer" class="transition-box">
            <div class="title-warp">
              <div class="close-btn" @click="turnClass">
    ...
    <!--调课-->
    <transition name="el-fade-in-linear">
      <div v-show="classCourseLayer" class="bg-layer">
        <transition name="el-zoom-in-center">
          <div v-if="classCourseLayer" class="transition-box">
            <div class="title-warp">
              <div class="close-btn" @click="couserClass">
                <i class="el-icon-close"></i>
              </div>
              <h3 class="modify-title">调课</h3>
            </div>
            <div class="couseTable">
              <v-turnclass :orderId="orderId" dType="tk" :attendclassList="attendclassList"></v-turnclass>
              <div style="float: right;margin-bottom: 20px;margin-right: 10px">
                <el-button type="primary" @click="classCourseLayer=!classCourseLayer" style="width: 80px" size="mini">取消</el-button>
              </div>
            </div>
    ...
    <!--退费-->
    <transition name="el-fade-in-linear">
      <div v-show="classMoneyLayer" class="bg-layer1">
        <transition name="el-zoom-in-center">
          <div v-if="classMoneyLayer" class="transition-box1">
            <div class="title-warp1">
              <div class="close-btn" @click="classMoneyLayer=!classMoneyLayer">
                <i class="el-icon-close"></i>
              </div>
            ...
            <div style="width:100%; height:680px; overflow-y:auto;padding-right:20px;">
              ...
              <div class="keci-list">
                <ul>
                  <li v-for="(item, index) in moneyList" :key="index" :class="{finish: item.roll_call_status === '01702' || item.roll_call_status === '01703' ||item.is_mend_register === true || item.isattend === 'f'}">
                    <p style="margin-top:15px;"></p>
                    {{item.class_sequence
                    < 10 ? '0' + item.class_sequence : item.class_sequence}}</li> </ul> </div> <div class="line">
              </div>
              <el-form :model="ruleForm" :rules="rules" size="mini" ref="ruleForm" label-width="100px" style="color:#333333; height: 540px; overflow-y: auto;">
                <el-row>
                  <el-row>
                    <el-col :span="12">
                  ...
                  <el-row>
                    <el-col :span="12">
                      <el-form-item label="额外退费">
                        <el-input style="width: 80px;" type="number" @mousewheel.native.prevent="(() => false)" placeholder="请输入内容" v-model="otherRefundAmount">
                        </el-input>
                        <el-input style="width: 192px;" type="text" placeholder="请输入内容" v-model="other_refund_remark">
                    ...
                    <el-col :span="12">
                      <el-form-item label="退费总额">
                        <span>{{(+classRefundAmount + +otherRefundAmount).toFixed(2)}}</span>
                      </el-form-item>
                    </el-col>
                  </el-row>
                  <el-row>
                  ...
                  <el-form-item label="退课原因">
                    <el-select style="width: 50%;" v-model="ruleForm.refundReason" placeholder="请选择">
                ...
                <el-form-item style="display: flex; justify-content: center; margin-left: -100px;">
                  ...
                  <el-button type="primary" @click="classMoneyLayer=!classMoneyLayer" style="width: 120px">取消</el-button>
                </el-form-item>
              </el-form>
            </div>
          </div>
    ...
    <!--打印-->
    <transition name="el-fade-in-linear">
      <div v-show="printLayer" class="bg-layer">
        <transition name="el-zoom-in-center">
          <div v-if="printLayer" class="transition-box">
            <div class="title-warp">
              <div class="close-btn" @click="printLayer=!printLayer">
                <i class="el-icon-close"></i>
              </div>
              <h3 class="modify-title">打印听课证</h3>
            </div>
            <div style="width:100%; height: 700px;overflow-y: auto; padding-right: 20px;">
              <!--startprint-->
              <div id="box">
                <div class="s_center">
              ...
              <!--endprint-->
              <el-button size="medium" type="primary" style="width: 200px;float: right;margin-right: 70px;margin-top: 40px" @click="cencelPrint">取消</el-button>
              <el-button size="medium" type="primary" style="width: 200px;float: right;margin-right: 70px;margin-top: 40px" @click="doPrint">打印</el-button>
            </div>
          </div>
        </transition>
      </div>
...
import VueQArt from "../QRCode/QRCode";
export default {
  components: {
    VTurnclass,
    VPayfees,
  ...
  data() {
    return {
      ...
      role_name: "",
      status: "",
      otherRefundAmount: null, //额外退费
      other_refund_remark: "",
      organ_id2: [], //新增组件内的学校id,
      attendList: [], //上课信息
      attendclassList: [], //课次信息
      dataList: [], //请求的list
      roleIds: [],
      location: [], //校区导航
      selectedLocation: [], //选择校区
      multipleSelection: [], //选中行的集合
      changeschedulelist: [], //调课集合
      moneyList: [], //退费
      addTeamLayer: false, //新增弹出框状态
      changeTeamLayer: false, //修改弹出框状态
      classTimeLayer: false, //选择课次弹出控制
      classCourseLayer: false, //调课
      classMoneyLayer: false, //退费
      turnClassSwitch: false, //转班
      printLayer: false, //打印听课证
      checkinNum: 1,
      signupOrder: [],
      printInfo: "", //打印信息
      liberalType: "",
      ...
      getRefundInfoRes: "" // 存放当前退费信息
    };
  },
  //实例创建完成后被立即调用
  created() {
    const self = this;
    this.$store.commit("isTurnClassSwitch", false);
  ...
  computed: {
    ...
    data() {
      ...
      return self.dataList.filter(function(d) {
        ...
        d.order_status = self.Util.GetLabelForValue(
          self.$store.state.orderStatus,
          d.orderstatus
        );
        return d;
      });
  ...
  watch: {
    selectedLocation: function() {
      if (
        ...
        this.selectedLocation.includes(+sessionStorage.areaNavParentId)
      ) {
    ...
    refundTimes: function() {
      if (this.refundTimes > this.buyTimes) {
        this.$message({
          type: "error",
          message: "退款次数大于购买次数"
        });
        this.refundTimes = this.buyTimes;
      }
      this.returnSinglePrice = this.signupOrder.single_price * this.refundTimes;
      this.classRefundAmount = (
        this.getRefundInfoRes.singlePrice * this.refundTimes +
        +this.otherRefundAmount
      ).toFixed(2);
    }
  },
  methods: {
        ...
        .then(
          res => {
            if (res.data.code === "10000") {
              ...
              self.attendList = res.data.classinfo;
              this.attendclassList = res.data.checkinlist;
              this.$store.commit("setClassCourseInfo", self.attendList);
              /*let timeOld = parseInt( self.attendList.left_times)-parseInt(self.attendList.left_times)+1;
                                              for(let i=timeOld; i<=parseInt(self.attendList.left_times); i++){
              ...
              //this.$router.push({path: '/studentInfo/classRecord'});
            } else {
              self.$message({
                type: "error",
                message: res.data.errmsg
    ...
    handlemoney(index, row) {
      const self = this;
      this.rowInfo = row;
      this.ruleForm.refundSinglePrice = row.single_price;
      this.ruleForm.classRefundAmount = row.single_price * row.left_times;
      let objData = {
        staffId: JSON.parse(window.sessionStorage.getItem("staff")).staffId,
        source: "PC",
        orderId: this.rowInfo.order_id //订单编号
      ...
      this.$axios
        ...
        .then(
          res => {
            if (res.data.code === "10000") {
              this.classMoneyLayer = true;
              self.getRefundInfoRes = res.data;
              // self.getRefundInfoRes.singlePrice = self.getRefundInfoRes.singlePrice.toFixed(2);
              self.moneyList = res.data.stuCheckinItem;
              self.checkinNum = res.data.checkinNum;
              self.signupOrder = res.data.signupOrder;
              self.liberalType = this.Util.GetLabelForValue(
                self.$store.state.LiberalTypeOptions,
                res.data.liberalType
              );
              this.buyTimes = res.data.buyTimes;
    ...
    submitMoneyForm(ruleForm, qx = false) {
      ...
      if (+this.otherRefundAmount !== 0) {
        if (!this.other_refund_remark) {
          ...
          +this.refundTimes !==
          this.signupOrder.buy_times - this.checkinNum
        ) {
      ...
      this.$refs[ruleForm].validate(valid => {
        if (valid) {
          ...
          let objData = {
            ...
            checkinNum: this.checkinNum, //签到次数
            buyTimes: this.signupOrder.buyTimes, //购买次数
    ...
    turnClass() {
      this.classTimeLayer = false;
      this.$store.commit("setTurnSwitch", false);
    },
    couserClass() {
      this.classCourseLayer = false;
      this.$store.commit("setCouserSwitch", false);
    },
    handleOPencourse(index, row) {
      const self = this;
      let objData1 = {
        source: this.GlobalVal.constants.source, //请求方式   Wechat，Android，PC，IOS
        staffId: JSON.parse(window.sessionStorage.getItem("staff")).staffId,
      ...
      this.$axios
        .post(
          this.GlobalVal.httpLink.updateOrderStatus,
