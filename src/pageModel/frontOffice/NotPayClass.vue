...
<template>
  <div class="class-query table2">
    <div class="handle-box" style="margin-bottom: 10px;">
      <el-button type="primary" @click="oneKeyPayment" size="mini">一键缴费</el-button>
    </div>
    <el-table size="mini" :data="data" border style="width: 100%" ref="multipleTable" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="class_name" label="班级名称" width="300">
      ...
      <el-table-column label="操作" width="200">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="primary" @click="handleEdit(scope.$index, scope.row)">缴费
          </el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <transition name="el-zoom-in-center">
          <div v-if="paymentLayer" class="transition-box">
            <div class="title-warp">
              <div class="close-btn" @click="paymentLayer=!paymentLayer">
            ...
            <div class="lesson-single">
              <div class="lesson-single-warp">
                <v-payfees @handleClosed="handleClosed"></v-payfees>
              </div>
            </div>
          </div>
        </transition>
  ...
  computed: {
    ...
    data() {
      ...
      return self.dataList.filter(function(d) {
        ...
        d.start_date = self.Util.TimeCycle(
          new Date(d.start_date).getTime(),
          "ymd"
        );
        d.end_date = self.Util.TimeCycle(new Date(d.end_date).getTime(), "ymd");
  ...
  methods: {
    ...
    oneKeyPayment() {
      if (!this.dataList.length) {
        this.$message.error("没有未缴费订单!");
        return;
      }
      if (
        !this.dataList.every(
          item => item.campus_id === this.dataList[0].campus_id
        )
      ) {
        this.$message.error("未缴费订单不在一个校区，无法一键缴费！");
        return;
      }
      this.handleEdit("", {
    ...
    handleEdit(index, row) {
      ...
      let objData = {
        staffId: JSON.parse(window.sessionStorage.getItem("staff")).staffId,
        source: this.GlobalVal.constants.source, //请求方式   Wechat，Android，PC，IOS
        studentId: this.rowInfo.student_id, //学生id，可以为空
        orderId: this.rowInfo.order_id //订单 ID，可以为空
      };
      console.log("查询查詢未交费订单发起请求的参数", objData);
      this.$axios
        .post(this.GlobalVal.httpLink.getnotpaidorder, JSON.stringify(objData))
        .then(
          res => {
            if (res.data.code === "10000") {
              console.log("查詢未交费订单获取的对象", res.data);
              self.count = res.data.count;
              self.$store.commit(
