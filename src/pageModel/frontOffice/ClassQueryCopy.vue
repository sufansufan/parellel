  ...
  <div class="class-query table">
    <div class="handle-box">
      ...
      <div class="class-query-screen">
        ...
        <el-cascader clearable size="mini" expand-trigger="click" :options="location" v-model="selectedLocation" placeholder="学校不限" change-on-select @change="handleLocationChange" style="width:267px">
        </el-cascader>
        <!--change-on-select-->
      ...
      <div class="search-warp">
        <el-input size="mini" v-model="select_word" :fetch-suggestions="queryTeacherOrClassname" placeholder="请输入搜索内容" class="search-input">
        ...
        <el-button @click="exportClass" size="mini" type="primary" v-show="$store.state.code.includes('010301')">导出班级</el-button>
        <el-button @click="resetCondition" size="mini" type="primary">重置搜索条件</el-button>
      </div>
      <!--条件点击选择框-->
    ...
    <el-table size="mini" :data="data" stripe style="width: 100%" ref="multipleTable" v-loading="loading" @selection-change="handleSelectionChange">
                ...
                <template slot-scope="props">
                    <el-form size="mini" label-position="left" inline class="demo-table-expand">
                        ...
                        <el-form-item label="科目">
                            <span>{{ props.row.subjectComputed }}</span>
                        </el-form-item>
                    </el-form>
                </template>
            </el-table-column>-->
      <el-table-column prop="campus_name" label="校区" fixed></el-table-column>
      <el-table-column label="班级名称" width="250" fixed>
        <template slot-scope="scope">
          <el-button @click="showDetails(scope.row)" type="text" size="small" style="white-space: normal;">
            {{ scope.row.class_name }}
          </el-button>
        </template>
      </el-table-column>
      <el-table-column label="老师" fixed>
        <template slot-scope="scope">
          <el-button @click="showTeacher(scope.row)" type="text" size="small">
      ...
      <el-table-column label="教室">
        <template slot-scope="scope">
          <div v-if="scope.row.classroom_name == '未找到'"></div>
          <div v-else>{{scope.row.classroom_name}}</div>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="time_name" width="100" label="时间段"></el-table-column>
      <el-table-column prop="start_date" label="上课日期" width="95"></el-table-column>
      ...
      <!--<el-table-column label="操作" width="300">
                <template slot-scope="scope">
                    ...
                    <el-button size="mini" type="primary"
                               @click="enList(scope.$index, scope.row)">报名
                    </el-button>
                    <el-button v-if="!scope.row.status" type="primary" size="mini" @click="handleUp(scope.$index, scope.row)">上架</el-button>
                    <el-button v-if="scope.row.status" size="mini" type="primary" @click="handleDown(scope.$index, scope.row)">下架</el-button>
                    <el-button size="mini" type="primary"
    ...
    <div class="pagination">
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" layout="slot, total, sizes, prev, pager, next, jumper" :current-page="cur_page + 1" :total="count" :page-size="cur_limit" :page-sizes="[10,20,30,50]">
        <slot name="slot">
          <span>
            <span style="padding-right: 10px;">在班总人次：{{preStudentNum || 0}}人</span>
            <span style="padding-right: 10px;">在班总人数：{{realStudentNum || 0}}人</span>
          </span>
        </slot>
      </el-pagination>
    </div>
    ...
    <ClassDetail v-if="reLoad" ref="classDetailBox" :details="classDetail"></ClassDetail>
    <!-- 教师详情 -->
    <Dialog ref="dialog" title="教师详情" titbg>
      <div class="dialog-box">
        ...
        <el-row :gutter="24">
          <el-col :span="8">
            <div class="dialogContent">
              <div>姓名</div>
              <el-input disabled :value="teacherList.teacher_name"></el-input>
            </div>
          </el-col>
          <el-col :span="8">
            <div class="dialogContent">
              <div>工号</div>
              <el-input disabled :value="teacherList.teacher_no"></el-input>
            </div>
          </el-col>
          <el-col :span="8">
            <div class="dialogContent">
              <div>授课科目</div>
              <el-input disabled :value="teacherList.subject"></el-input>
            </div>
          </el-col>
        </el-row>
        <el-row :gutter="24">
          <el-col :span="8">
            <div class="dialogContent">
              <div>邮箱</div>
              <el-input disabled :value="teacherList.teacher_email"></el-input>
            </div>
          </el-col>
          <el-col :span="8">
            <div class="dialogContent">
              <div>岗位性质</div>
              <el-input disabled :value="teacherList.post_nature"></el-input>
            </div>
          </el-col>
          <el-col :span="8">
            <div class="dialogContent">
              <div>所属校区</div>
              <el-input disabled :value="teacherList.campus_name"></el-input>
            </div>
          </el-col>
        </el-row>
        <b>更多信息</b>
        <el-row :gutter="24">
          <el-col :span="4">
            <div class="dialogContent">
              <div>性别</div>
              <el-input disabled :value="teacherList.sex"></el-input>
            </div>
          </el-col>
          <el-col :span="4">
            <div class="dialogContent">
              <div>年龄</div>
              <el-input disabled :value="teacherList.age"></el-input>
            </div>
          </el-col>
          <el-col :span="4">
            <div class="dialogContent">
              <div>出生年月</div>
              <el-input disabled :value="teacherList.birthday"></el-input>
            </div>
          </el-col>
          <el-col :span="4">
            <div class="dialogContent">
              <div>民族</div>
              <el-input disabled :value="teacherList.nation_name"></el-input>
            </div>
          </el-col>
          <el-col :span="4">
            <div class="dialogContent">
              <div>政治面貌</div>
              <el-input disabled :value="teacherList.political_affiliation"></el-input>
            </div>
          </el-col>
        </el-row>
        <b>教师资历</b>
        <el-row :gutter="24">
          <el-col :span="4">
            <div class="dialogContent">
              <div>教师年限</div>
              <el-input disabled :value="teacherList.teach_life"></el-input>
            </div>
          </el-col>
          <el-col :span="4">
            <div class="dialogContent">
              <div>毕业院校</div>
              <el-input disabled :value="teacherList.graduate_school"></el-input>
            </div>
          </el-col>
          <el-col :span="8">
            <div class="dialogContent">
              <div>专业</div>
              <el-input disabled :value="teacherList.self_subject"></el-input>
            </div>
          </el-col>
          <el-col :span="8">
            <div class="dialogContent">
              <div>最高学历</div>
              <el-input disabled :value="teacherList.highest_education_degree"></el-input>
            </div>
          </el-col>
        </el-row>
        <el-row :gutter="24">
          <el-col :span="24">
            <div class="dialogContent">
              <div>教师简介</div>
              <el-input disabled type="textarea" rows="3" :value="teacherList.brief_introduce"></el-input>
            </div>
          </el-col>
        </el-row>
        <el-row :gutter="24">
          <el-col :span="24">
            <div class="dialogContent">
              <div>教学特点</div>
              <el-input disabled type="textarea" rows="3" :value="teacherList.teach_character"></el-input>
            </div>
          </el-col>
        </el-row>
        <el-row :gutter="24">
          <el-col :span="24">
            <div class="dialogContent">
              <div>教学成果</div>
              <el-input disabled type="textarea" rows="3" :value="teacherList.teach_results"></el-input>
            </div>
          </el-col>
        </el-row>
      </div>
...
<script>
import ClassDetail from "./ClassDetails.vue";
...
export default {
  ...
  methods: {
    //重置搜索条件
    resetCondition() {
      this.resubmitClassOnly = false;
      this.notFullClassOnly = false;
      this.mondayOnly = false;
      this.tuesdayOnly = false;
      this.wednesdayOnly = false;
      this.thursdayOnly = false;
      this.fridayOnly = false;
      this.saturdayOnly = false;
      this.sundayOnly = false;
      this.morningOnly = false;
      this.afternoonOnly = false;
      this.nightOnly = false;
      this.selectTeacherOrClass = "teacher";
      this.projectType = ""; //项目类型 [长期，短期，公益，考试]等
      this.classType = ""; //班型，[超常，培优]等
      this.grade = ""; //适用年级，[一年级，二年级]等
      this.season = ""; //季节[暑假，秋季，寒假，春季]等
      this.periods = ""; //期别
      this.particular_year = null; //int 年份
      this.subject = ""; //科目，[数学，物理]等
      this.department = ""; //学部，小学，初中，高中
      this.className = ""; //班级名称
      this.teacherName = ""; //老师名字
      this.select_word = ""; //搜索的值
      this.cur_page = 0;
    ...
    showDetails(row) {
      ...
      this.$nextTick(() => {
        this.reLoad = true;
        setTimeout(() => {
          this.$refs.classDetailBox.centerDialogVisible = true;
        }, 0);
      });
    },
    showTeacher(row) {
      ...
      this.$axios
        .post(this.GlobalVal.httpLink.getTeacherById, JSON.stringify(objData))
        .then(
          res => {
            if (res.data.code === "10000") {
              this.$refs.dialog.dialogVisible = true;
              this.teacherList = res.data.data.teacher;
              this.teacherList.subject = this.teacherList.subject
                .split(",")
                .map(item => {
                ...
                .join(",");
              this.teacherList.sex =
                this.teacherList.sex === true ? "男" : "女";
              this.teacherList.birthday = this.Util.TimeCycle(
                new Date(this.teacherList.birthday).getTime(),
                "ymd"
              );
            } else {
              console.error(res);
    ...
    getData() {
      ...
      this.loading = true;
      this.$axios
        .post(this.GlobalVal.httpLink.queryClass, JSON.stringify(this.objData))
        .then(
          res => {
            if (res.data.code === "10000") {
              this.count = res.data.count;
              this.dataList = res.data.pxxClasses;
              this.preStudentNum = res.data.preStudentNum.toLocaleString();
              this.realStudentNum = res.data.realStudentNum.toLocaleString();
              this.dataList = this.dataList.map(item => {
                let val = this.$store.state.PeriodsOptions.filter(
                  val => val.value === item.periods
                )[0];
                item.periods = val && val.label;
                return item;
              });
            } else {
              console.error(res);
