...
/* .el-select .el-input .el-icon-arrow-up::before{
  content: '';
  line-height: 31px;
  border-bottom: 6px solid #419dfd;
  ...
  <div class="class-query table">
    <div class="handle-box">
      ...
      <div class="class-query-screen">
        ...
        <div>
          <el-input size="mini" v-model="oldTeacherOrClassName" :fetch-suggestions="oldQueryTeacherOrClassname" placeholder="请输入搜索内容" class="search-input">
      ...
      <div class="class-query-screen">
        <div>
          <h3 class="titleAttend">调入班级:</h3>
        </div>
        <div>
          <el-select v-model="newParticularYear" clearable size="mini" placeholder="年度">
            <el-option v-for="item in $store.state.YearOptions" :key="item.code" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </div>
        <div>
          <el-select v-model="newSeason" clearable size="mini" placeholder="季节">
            <el-option v-for="item in $store.state.SeasonOptions" :key="item.code" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </div>
        <div>
          <el-select v-model="newPeriods" clearable size="mini" placeholder="期别不限">
            <el-option v-for="item in $store.state.PeriodsOptions" :key="item.code" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </div>
        <div>
          <el-select v-model="newGrade" clearable size="mini" placeholder="年级不限">
            <el-option v-for="item in $store.state.GradeOptions" :key="item.code" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </div>
        <div>
          <el-select v-model="newSubject" clearable size="mini" placeholder="科目不限">
            <el-option v-for="item in $store.state.SubjectOptions" :key="item.code" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </div>
        <div>
          <el-select v-model="newProjectType" clearable size="mini" placeholder="类别不限">
            <el-option v-for="item in $store.state.ProjectTypeOptions" :key="item.code" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </div>
        <div>
          <el-select v-model="newClassType" clearable size="mini" placeholder="班型不限">
            <el-option v-for="item in $store.state.ClassTypeOptions" :key="item.code" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </div>
        <div>
          <el-cascader clearable size="mini" expand-trigger="click" :options="location" v-model="selectedLocation1" placeholder="学校不限" change-on-select @change="handleLocationChange">
          </el-cascader>
        </div>
        <div>
          <el-input size="mini" v-model="newTeacherOrClassName" :fetch-suggestions="newQueryTeacherOrClassname" placeholder="请输入搜索内容" class="search-input">
            <el-select style="width: 100px;" v-model="newSelectTeacherOrClass" slot="prepend" placeholder="请选择">
              <el-option label="老师姓名" value="teacher"></el-option>
              <el-option label="班级" value="class"></el-option>
            </el-select>
            <el-button class="search-btn" slot="append" icon="el-icon-search" @click="search"></el-button>
    ...
    <el-table size="mini" :data="data" stripe style="width: 100%" ref="multipleTable" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="oldclass_name" label="在读班级名称" width="280"></el-table-column>
      ...
      <el-table-column prop="oldsequence_datestring" label="课次日期" width="100"></el-table-column>
      <el-table-column prop="newcampus_name" label="现校区"></el-table-column>
      <el-table-column prop="newclass_name" label="调入的班级名称" width="280"></el-table-column>
      ...
      <el-table-column prop="newsequence_datestring" label="课次日期" width="100"></el-table-column>
      <el-table-column prop="remark" label="备注"></el-table-column>
      <el-table-column prop="staff_name" label="操作人"></el-table-column>
      <el-table-column prop="operate_time" label="操作时间" width="100"></el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      newSelectTeacherOrClass: "teacher",
      className: "", //班级名称
      teacherName: "", //老师名字
      teacherMobile: "", //老师电话
      select_word: "", //搜索的值
  ...
  methods: {
    getParams(pageClear) {
      this.objData = Object.assign({}, this.objData, {
        ...
        endTime: this.endTime
          ? self.Util.TimeCycle(this.endTime.getTime(), "ymd")
          : "", //转班时间结束范围
    ...
    exportList() {
      this.$exportExcel({
        ...
        httpUrl: "exportChangeClassRefund",
    ...
    getData(pageClear) {
      ...
      this.$axios
        .post(
          this.GlobalVal.httpLink.queryChangeClassScheduleHistory,
    ...
    queryTeacherOrClass(queryString, cb, type) {
      if (type == "class") {
        let objData = {
          staffId: JSON.parse(window.sessionStorage.getItem("staff")).staffId,
          page: 1, //页数
    ...
    // 搜索框动态提示内容
    oldQueryTeacherOrClassname(queryString, cb) {
      // this.queryTeacherOrClass(queryString, cb, this.selectTeacherOrClass)
    },
    // 搜索框动态提示内容
    newQueryTeacherOrClassname(queryString, cb) {
      // this.queryTeacherOrClass(queryString, cb, this.newSelectTeacherOrClass)
    }
  }
};
</script>
